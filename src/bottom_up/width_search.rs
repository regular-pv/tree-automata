...
pub trait SearchPattern<F: Symbol, Q: State, C: SearchContext>: Clone {
    fn matches(&self, context: &C, q: &Q, configuration: &Configuration<F, Q>) -> Option<(C, Vec<Self>)>;
}
...
impl<'a, E: ?Sized, F: Symbol, Q: LanguageState<F, E>, C: SearchContext, P: SearchPattern<F, Q, C>> TermFragment<'a, E, F, Q, C, P> {
    pub fn new(env: &'a E, context: C, leaves: Vec<Q>, patterns: Vec<P>, kill_signal: crossbeam_channel::Receiver<()>) -> TermFragment<'a, E, F, Q, C, P> {
        ...
        TermFragment {
            ...
            patterns: patterns,
            leaves: leaves,
            leaves_configurations: leaves_configurations,
            current: Vec::new(),
            next_fragment: None,
            visited: false,
            #[cfg(debug_assertions)]
...
impl<'a, E: ?Sized, F: Symbol, Q: LanguageState<F, E>, C: SearchContext, P: SearchPattern<F, Q, C>> Iterator for TermFragment<'a, E, F, Q, C, P> {
    ...
    fn next(&mut self) -> Option<Result<Vec<Term<F>>, Killed>> {
        ...
        loop {
            if let Ok(()) = self.kill_signal.try_recv() {
            ...
            if self.leaves.is_empty() {
                ...
                if self.current.len() < self.leaves.len() {
                    ...
                    match self.leaves_configurations.last_mut().unwrap().next() {
                        Some(conf) => {
                            if let Some((next_context, sub_patterns)) = self.patterns[i].matches(&current_context, &self.leaves[i], &conf) {
                    ...
                    match self.next_fragment {
                        ...
                        None => {
                            ...
                            next_fragment = TermFragment::new(self.env, current_context.clone(), leaves, patterns, self.kill_signal.clone());
                            {
                                #![cfg(debug_assertions)]
                                next_fragment.debug.depth = self.debug.depth + 1;
                            }
