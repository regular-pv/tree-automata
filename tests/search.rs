...
#[test]
fn simple_common_configurations() {
    let mut a = Automaton::new();

    let conf = Pattern::cons((), &[]);

    a.add(conf.clone(), (), true);
    a.add(conf.clone(), (), false);

    let automata = [&a, &a];
    let positions = [true.into(), false.into()];
    let patterns = [(), ()];

    let mut it = Automaton::common_configurations(
        &automata,
        &positions,
        &patterns,
        ()
...
//
//     a.add(Pattern::cons('f', &["ab".into()]), (), "f(ab)");
//     a.add(Pattern::cons('f', &["bc".into()]), (), "f(bc)");
//
//     let automata = [&a, &a];
//     let positions = ["f(ab)".into(), "f(bc)".into()];
//     let patterns = [(), ()];
//     let constraints = ();
//
//     let term = Term::new('f', &[Term::new('b', &[])]);
//
//     let mut it = Automaton::synchronized_runs(
//         &automata,
//         &positions,
